...
>
> 但我们并未止步于此——通过架构的通用化设计，这款插件已完美兼容 NVIDIA (CUDA) 与 AMD (ROCm) 等主流平台。
>
...
### 🔮 工作流显存预测（PRED）
- 分析当前工作流中加载的所有模型，预测运行所需显存
- 综合考虑显存余量、系统内存、虚拟提交空间，给出成功率评估
- 颜色实时反馈：🟢 绿色（稳定）/ 🟡 黄色（警告）/ 🔴 红色（风险）
...
| **GPU 引擎** | 负载 %、核心频率、温度 |
| **VRAM** | 驱动层显存占用 / 总量 |
| **RSV** | PyTorch 缓存池（`memory_reserved`） |
| **PWR** | 瞬时功耗 / TGP 负载比例 |
| **CPU** | 占用率、主频 |
| **RAM** | 内存使用率 |

...
|------|----|------|
| $\alpha$ | 0.9 | 显存碎片化折扣 |
| $\gamma$ | 平台相关 | 硬约束平台系数（见下方） |

### 3.1 平台系数 $\gamma$

不同 GPU 平台的内存架构能力差异显著，硬约束计算需引入平台系数：

| 平台 | $\gamma$ | 说明 |
|------|----------|------|
| Intel Arc (XPU) | 1.0 | 无 UVM 或 UVM 支持较弱，硬约束严格 |
| NVIDIA (CUDA) | 4.0 | CUDA Unified Memory 支持显存溢出约 4x，硬约束放宽 |

**有效显存上限（平台调整后）：**

$$V_{eff}^{platform} = V_{eff} \times \gamma$$
...
$$P_{peak} = \begin{cases}
1.0 & D_{peak} = 0 \\[6pt]
\max\!\left(0.02,\ e^{-3 \cdot D_{peak} / V_{eff}^{platform}}\right) & D_{peak} > 0
\end{cases}$$

**曲线特性（Intel Arc, $\gamma=1.0$）：**
...
**曲线特性（NVIDIA, $\gamma=4.0$）：**

| $D_{peak}$ | 等效溢出比例 | $P_{peak}$ |
|-----------|-------------|-----------|
| 0 GB | 0% | 100% |
| $V_{eff} \times 1.0$ | 25% | ≈ 47% |
| $V_{eff} \times 2.0$ | 50% | ≈ 22% |
| $V_{eff} \times 3.0$ | 75% | ≈ 11% |
| $V_{eff} \times 4.0$ | 100% | ≈ 5% |
...
6. **平台系数 $\gamma$ 为经验值**：NVIDIA 的 4.0 基于典型 UVM 行为估算，实际能力因驱动版本、PCIe 带宽、系统内存速度而异；极端情况下（如系统内存紧张）实际成功率可能低于预测
